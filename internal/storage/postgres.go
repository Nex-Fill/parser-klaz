...
func (p *Postgres) UpsertAd(ctx context.Context, ad *kl.Ad) error {
	_, err := p.pool.Exec(ctx, `
		INSERT INTO ads (id, title, description, price, price_eur, contact_name,
			...
			shipping_type, item_condition, attributes, zip_code, city, latitude, longitude)
		...
		ON CONFLICT (id) DO UPDATE SET
			...
			attributes = CASE WHEN EXCLUDED.attributes != '{}' THEN EXCLUDED.attributes ELSE COALESCE(ads.attributes, '{}') END,
			zip_code = CASE WHEN EXCLUDED.zip_code != '' THEN EXCLUDED.zip_code ELSE ads.zip_code END,
			city = CASE WHEN EXCLUDED.city != '' THEN EXCLUDED.city ELSE ads.city END,
			latitude = CASE WHEN EXCLUDED.latitude != 0 THEN EXCLUDED.latitude ELSE ads.latitude END,
			longitude = CASE WHEN EXCLUDED.longitude != 0 THEN EXCLUDED.longitude ELSE ads.longitude END,
		...
		ad.ShippingType, ad.ItemCondition, ad.Attributes, ad.ZipCode, ad.City, ad.Latitude, ad.Longitude,
	)
	return err
}
...
func (p *Postgres) BatchUpdateCounters(ctx context.Context, views map[string]int, favorites map[string]int) error {
	...
	for id, v := range views {
		...
		if v > 0 {
			er := float64(0)
			if v >= 10 && f > 0 {
				er = math.Min(float64(f)/float64(v)*100, 100)
			}
			ds := math.Min(100, math.Min(float64(f)/math.Max(float64(v), 1)*100, 20)*2.5+math.Min(float64(f), 100)*0.5)
			batch.Queue(`UPDATE ads SET views = $2, favorites = $3, last_checked_at = NOW(), updated_at = NOW(),
				engagement_rate = $4, demand_score = $5
				WHERE id = $1`, id, v, f, er, ds)
		} else {
...
func (p *Postgres) GetAd(ctx context.Context, id string) (*kl.Ad, error) {
	...
	err := p.pool.QueryRow(ctx, `
		SELECT id, title, description, price, price_eur, contact_name,
			...
			COALESCE(attributes, '{}'),
			COALESCE(zip_code, ''), COALESCE(city, ''), COALESCE(latitude, 0), COALESCE(longitude, 0)
		...
		&ad.AdType, &ad.PriceType, &ad.BuyNowSelected, &ad.BuyNowPrice, &ad.UserRating, &ad.ImageURLs,
		&ad.ShippingType, &ad.ItemCondition, &ad.Attributes,
		&ad.ZipCode, &ad.City, &ad.Latitude, &ad.Longitude,
	)
	if err != nil {
		return nil, err
	}
